/**
 * Script: sync-r2-assets.ts
 * Purpose: Synchronize 75 template backgrounds and thumbnails from CineLove / local cache to Cloudflare R2
...
const LOCAL_TEMPLATES_DIR = path.join(process.cwd(), "apps/web/public/templates");
const DOCS_DIR = path.join(process.cwd(), "docs");

interface SyncResult {
  filename: string;
  sizeBytes: number;
  status: "uploaded" | "skipped_already_exists" | "verified_local" | "failed";
  r2Key?: string;
  error?: string;
}
