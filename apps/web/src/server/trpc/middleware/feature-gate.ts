...
/**
 * Assert that the user's plan supports premium templates.
 * Used by template selection and certain editor features.
 */
export function assertPremiumAccess(
  limits: (typeof PLAN_LIMITS)[PlanTier],
  featureName: string,
): void {
  if (!limits.hasPremiumTemplates) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: `"${featureName}" chỉ dành cho gói Premium. Nâng cấp để sử dụng!`,
    });
  }
}
