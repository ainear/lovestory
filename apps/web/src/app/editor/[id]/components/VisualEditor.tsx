...
const TEXT_PRESETS = [
  ...
  {
    label: "Ghi chú",
    text: "Nhập ghi chú của bạn tại đây",
    fontSize: 13,
...
const BG_PRESETS = [
  ...
  {
    label: "Xanh biển",
    value: "linear-gradient(180deg, #ecfeff 0%, #cffafe 40%, #a5f3fc 100%)",
  },
  {
    label: "Xanh lá",
    value: "linear-gradient(180deg, #ecfdf5 0%, #d1fae5 40%, #a7f3d0 100%)",
  },
];
...
const MUSIC_PRESETS = [
  ...
  },
  {
    ...
    emoji: "🎬",
    genre: "Cinematic",
    url: "https://cdn.pixabay.com/audio/2024/02/15/audio_8b56c8c4fb.mp3",
  },
  {
    id: "m8",
    label: "Sweet Jazz",
  ...
  },
  {
    ...
    emoji: "☁️",
    genre: "Lofi",
    url: "https://cdn.pixabay.com/audio/2024/05/16/audio_166b80e398.mp3",
  },
  {
    id: "m10",
    label: "Acoustic Love",
    emoji: "🎸",
    genre: "Acoustic",
    url: "https://cdn.pixabay.com/audio/2024/01/08/audio_58cbc3aa64.mp3",
  },
  {
    id: "m11",
    label: "Piano Ballad",
    emoji: "🎹",
    genre: "Piano",
    url: "https://cdn.pixabay.com/audio/2023/05/13/audio_3f3e6e34cd.mp3",
  },
  {
    id: "m12",
    label: "Jazz Café",
    emoji: "☕",
    genre: "Jazz",
    url: "https://cdn.pixabay.com/audio/2023/09/04/audio_60bf8ded52.mp3",
  },
  {
    id: "m13",
    label: "Cinematic Emotion",
    emoji: "🎥",
    genre: "Cinematic",
    url: "https://cdn.pixabay.com/audio/2023/07/07/audio_3a53a4ea90.mp3",
  },
  {
    id: "m14",
    label: "Soft Pop Love",
    emoji: "💖",
    genre: "Pop",
    url: "https://cdn.pixabay.com/audio/2023/10/18/audio_69f8b44fa3.mp3",
  },
  {
    id: "m15",
    ...
    emoji: "💃",
    genre: "Cổ điển",
    url: "https://cdn.pixabay.com/audio/2023/03/15/audio_c94e0a0d37.mp3",
  },
  {
    id: "m16",
    label: "Lofi Sunset",
    emoji: "🌇",
    genre: "Lofi",
    url: "https://cdn.pixabay.com/audio/2024/03/20/audio_9c7a3e6bfa.mp3",
  },
];
...
export function VisualEditor({
  ...
  const [gridPattern, setGridPattern] = useState<"dots" | "lines" | "cross">(
    "dots",
  );
  ...
  const save = useCallback(async () => {
    ...
    state.introEffect,
    state.musicIconStyle,
    state.entranceAnimation,
    projectId,
    supabase,
    musicUrl,
    musicName,
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      // Sprint 21: Zoom shortcuts (Ctrl+/− and Ctrl+0)
        ...
        e.preventDefault();
        dispatch({ type: "SET_ZOOM", zoom: Math.min(200, state.zoom + 25) });
      } else if (ctrl && e.key === "-") {
        e.preventDefault();
        dispatch({ type: "SET_ZOOM", zoom: Math.max(25, state.zoom - 25) });
  ...
  const [bgRemoveElementId, setBgRemoveElementId] = useState<string | null>(
    null,
  );
    ...
    >
      ...
      {contextMenu &&
        (() => {
          ...
          const items = [
            {
              label: "📋 Nhân bản",
              action: () => dispatch({ type: "DUPLICATE", id: ctxEl.id }),
            },
            {
              label: "🗑️ Xóa",
              action: () => dispatch({ type: "DELETE_ELEMENT", id: ctxEl.id }),
              danger: true,
            },
            {
              label: ctxEl.locked ? "🔓 Mở khóa" : "🔒 Khóa vị trí",
              action: () =>
                dispatch({
                  type: "UPDATE_ELEMENT",
                  id: ctxEl.id,
                  changes: { locked: !ctxEl.locked },
                }),
            },
            {
              label: "⬆️ Lên trước",
              action: () => dispatch({ type: "BRING_FORWARD", id: ctxEl.id }),
            },
            {
              label: "⬇️ Xuống sau",
              action: () => dispatch({ type: "SEND_BACKWARD", id: ctxEl.id }),
            },
            // Sprint 44D: AI BG Remove (images only)
            ...(ctxEl.type === "image" && ctxEl.props.src
              ? [
                  {
                    label: "🪄 Xóa nền AI",
                    action: () => {
                      setBgRemoveImageUrl(ctxEl.props.src!);
                      setBgRemoveElementId(ctxEl.id);
                    },
                  },
                ]
              : []),
      ...
      >
          ...
          {ZOOM_LEVELS.map((z) => (
            <button
              ...
              style={{
                ...
                fontSize: 12,
                fontWeight: 500,
                color: state.zoom === z ? "#374151" : "#9ca3af",
                cursor: "pointer",
              }}
            >
              {z}%
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
          <button
            ...
            title={`Tải thiệp dạng ${exportFormat.toUpperCase()}`}
            style={{
              display: "flex",
              alignItems: "center",
              gap: 6,
        ...
        {(() => {
          ...
          const widgets = state.elements.filter(
            (e) => e.type === "widget",
          ).length;
          return (
            <div
              title={`${texts} text · ${images} image · ${widgets} widget`}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 6,
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        {activeTab !== "" && (
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
                        ...
                        dispatch({
                          ...
                          element: {
                            ...
                            props: {
                              ...
                              color:
                                state.background.includes("0f0825") ||
                                state.background.includes("111827")
                                  ? "#ffffff"
                                  : "#1f2937",
                  ...
                  {[
                    ...
                    },
                    {
                      ...
                      texts: [
                        ...
                        {
                          text: "Quý khách đến dự bữa tiệc",
                          fontSize: 16,
                          fontFamily: "'Dancing Script', cursive",
                          fontWeight: "700",
                          fontStyle: "normal",
                        ...
                        cluster.texts.forEach((t, i) => {
                          ...
                          dispatch({
                            ...
                            element: {
                              ...
                              y: baseY + t.dy,
                              width: 350,
                              height: t.fontSize + 20,
                              rotation: 0,
                              opacity: 1,
                              zIndex: state.elements.length + 1 + i,
                              locked: false,
                              props: {
                                text: t.text,
                                fontSize: t.fontSize,
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: 9, color: "#9ca3af", margin: 0 }}>
                          {cluster.texts.length} phần tử • click để thêm
                        </p>
                      </div>
                    </button>
                  ))}
              ...
              {/* STOCK TAB — migrated to CraftVisualEditor */}
                    ...
                    [
                      {
                        ...
                        desc: "Hiển thị lịch tháng với highlight ngày cưới",
                        props: {
                          widgetType: "calendar",
                          label: "Lịch cưới",
                      ...
                      },
                      {
                        ...
                        desc: "Hiển thị thời gian đến ngày cưới",
                        props: {
                      ...
                      },
                      {
                        ...
                        emoji: "🗺️",
                        label: "Bản đồ",
                        desc: "Gắn link Google Maps địa điểm",
                        props: {
                          widgetType: "map",
                      ...
                      },
                      {
                        ...
                        emoji: "📱",
                        label: "Mã QR",
                        desc: "QR code link thiệp để chia sẻ",
                        props: { widgetType: "qr", label: "Quét để xem thiệp" },
                        w: 160,
                        h: 180,
                      },
                      {
                        ...
                        desc: "Tài khoản nhận phong bì mừng cưới",
                        props: {
                          widgetType: "gift",
                          label: "Phong bì mừng cưới",
                      ...
                      },
                      {
                        ...
                        emoji: "📝",
                        label: "RSVP Form",
                        desc: "Form xác nhận tham dự tiệc cưới",
                        props: {
                          widgetType: "rsvp",
                          label: "RSVP",
                      ...
                      },
                      {
                        ...
                        emoji: "▶️",
                        label: "YouTube Video",
                        desc: "Nhúng video YouTube vào thiệp",
                        props: {
                          widgetType: "youtube",
                          label: "Video cưới",
                          youtubeUrl: "",
                      ...
                      },
                      {
                        ...
                        emoji: "📞",
                        label: "Nút gọi",
                        desc: "Nút bấm gọi điện cho cô dâu/chú rể",
                        props: {
                          widgetType: "call",
                          label: "Liên hệ cô/chú rể",
                      ...
                      },
                      {
                        ...
                        emoji: "🖼️",
                        label: "Album ảnh",
                        desc: "Album ảnh cưới dạng lưới/slider",
                        props: {
                          widgetType: "album",
                      ...
                      },
                      {
                        ...
                        emoji: "👤",
                        label: "Tên khách mời",
                        desc: "Tên tự động thay đổi khi gửi thiệp hàng loạt",
                        props: {
                          widgetType: "guestname",
                          label: "Tên khách mời",
                          guestNameLabel: "Trân trọng kính mời",
                        },
                        w: 320,
                        h: 140,
                      },
                  ...
                  >
                    {[
                      ...
                      },
                      {
                        id: "bg2",
                        label: "Hoa lavender",
                        url: "https://images.unsplash.com/photo-1490750967868-88df5691cc35?w=800",
                      },
                      {
                        id: "bg3",
                        label: "Vải lụa",
                      ...
                      },
                      {
                        id: "bg4",
                        label: "Bokeh vàng",
                        url: "https://images.unsplash.com/photo-1451847251646-8a6c0dd1510c?w=800",
                      },
                      {
                        id: "bg5",
                        label: "Mây trời",
                      ...
                      },
                      {
                        id: "bg6",
                        label: "Hoa trắng",
                        url: "https://images.unsplash.com/photo-1487530811015-780c5b3ac781?w=800",
                      },
                      {
                        id: "bg7",
                        label: "Marble",
                      ...
                      },
                      {
                        id: "bg8",
                        label: "Watercolor",
                        url: "https://images.unsplash.com/photo-1579762715118-a6f1d4b934f1?w=800",
                      },
                      {
                        id: "bg9",
                        label: "Hoàng hôn",
                      ...
                      },
                      {
                        id: "bg13",
                        label: "Lá xanh",
                        url: "https://images.unsplash.com/photo-1444653614773-995cb1ef9efa?w=800",
                      },
                      {
                        id: "bg14",
                        label: "Đêm sao",
                        url: "https://images.unsplash.com/photo-1465101162946-4377e57745c3?w=800",
                      },
                      {
                        id: "bg15",
                        label: "Cherry",
                        url: "https://images.unsplash.com/photo-1522566220397-7127240a0b67?w=800",
                      },
                      {
                        id: "bg16",
                        label: "Confetti",
                      ...
                      >
                        ...
                        <img
                          src={bg.url.replace("w=800", "w=200")}
                          alt={bg.label}
                          style={{
                            width: "100%",
                            height: 60,
              ...
              {activeTab === "effects" && (
                ...
                >
                  ...
                  {[
                    {
                      label: "🌸 Cánh hoa",
                      effect: "petals" as ParticleEffect,
                    },
                    {
                      label: "💕 Trái tim",
                      effect: "hearts" as ParticleEffect,
                    },
                    { label: "✨ Bokeh", effect: "bokeh" as ParticleEffect },
                    { label: "❄️ Tuyết rơi", effect: "snow" as ParticleEffect },
                    {
                      label: "🚫 Tắt hiệu ứng",
                      effect: "none" as ParticleEffect,
                    },
                  ].map((fx) => (
                    <button
                      key={fx.effect}
                      ...
                      style={{
                        ...
                        background:
                          state.particleEffect === fx.effect
                            ? "#fdf2f8"
                            : "#fff",
                        cursor: "pointer",
                        fontSize: 13,
                  ...
                  {[
                    {
                      label: "✉️ Phong bì mở",
                      effect: "envelope" as IntroEffect,
                      desc: "Hiệu ứng mở phong bì cưới",
                    },
                    {
                      label: "🌅 Fade In",
                      effect: "fade" as IntroEffect,
                      desc: "Mờ dần hiện ra",
                    },
                    {
                      label: "📜 Slide Up",
                      effect: "slide" as IntroEffect,
                      desc: "Trượt từ dưới lên",
                    },
                    {
                      label: "🎭 Curtain Hồng",
                      effect: "curtainPink" as IntroEffect,
                      desc: "Rèm cửa hồng mở ra",
                    },
                    {
                      label: "🎭 Curtain Xanh",
                      effect: "curtainBlue" as IntroEffect,
                      desc: "Rèm cửa xanh mở ra",
                    },
                    {
                      label: "🎭 Curtain Vàng",
                      effect: "curtainGold" as IntroEffect,
                      desc: "Rèm cửa vàng kim mở ra",
                    },
                    {
                      label: "🚫 Không",
                      effect: "none" as IntroEffect,
                      desc: "Không hiệu ứng mở đầu",
                    },
                  ].map((fx) => (
                    <button
                      key={fx.effect}
                      ...
                      style={{
                        ...
                        background:
                          state.introEffect === fx.effect ? "#fdf2f8" : "#fff",
                        cursor: "pointer",
                        fontSize: 13,
                        color: "#374151",
                        display: "flex",
                    ...
                    >
                      ...
                      {state.introEffect === fx.effect && (
                        <span style={{ color: "#ff6b9d", fontSize: 12 }}>
                          ✔
                        </span>
                      )}
                    </button>
                  ...
                  >
                    {[
                      {
                        label: "None",
                        animation: "none" as EntranceAnimation,
                        icon: "⊘",
                      },
                      {
                        label: "Fade In All",
                        animation: "fadeInAll" as EntranceAnimation,
                        icon: "◐",
                      },
                      {
                        label: "Slide Up All",
                        animation: "slideUpAll" as EntranceAnimation,
                        icon: "△",
                      },
                      {
                        label: "Scale In All",
                        animation: "scaleInAll" as EntranceAnimation,
                        icon: "◇",
                      },
                      {
                        label: "Flip In All",
                        animation: "flipInAll" as EntranceAnimation,
                        icon: "⊡",
                      },
                      {
                        label: "Slide Up Mix",
                        animation: "slideUpMix" as EntranceAnimation,
                        icon: "▲",
                      },
                      {
                        label: "Fade In Mix",
                        animation: "fadeInMix" as EntranceAnimation,
                        icon: "◑",
                      },
                    ].map((fx) => (
                      <button
                        ...
                        style={{
                          ...
                          background:
                            state.entranceAnimation === fx.animation
                              ? "#fdf2f8"
                              : "#fff",
                          cursor: "pointer",
                          fontSize: 11,
                      ...
                      >
                        ...
                        {state.entranceAnimation === fx.animation && (
                          <span style={{ color: "#ff6b9d", fontSize: 10 }}>
                            ✔ Đang chọn
                          </span>
                        )}
                      </button>
              ...
              {activeTab === "music" && (
                ...
                >
                  ...
                  >
                    Nhạc nền ({MUSIC_PRESETS.length} bài)
                  </p>
                  {musicUrl && (
                    <div
                      style={{
                        ...
                        background: isPlaying
                          ...
                          : "#f9fafb",
                        border:
                          "1px solid " + (isPlaying ? "#ff6b9d" : "#e5e7eb"),
                        display: "flex",
                        alignItems: "center",
                        gap: 10,
                      }}
                  ...
                  >
                    {MUSIC_PRESETS.filter(
                      (m) => musicGenre === "Tất cả" || m.genre === musicGenre,
                    )
                      .filter(
                        (m) =>
                          !musicSearch ||
                      ...
                      .map((m) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() => handleSetMusic(m.url, m.label)}
                            style={{
                              flex: 1,
                              background: "none",
                              border: "none",
                  ...
                  <div style={{ marginTop: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      <input
                        ...
                        value={
                          musicUrl.startsWith("https://cdn.pixabay")
                            ? ""
                            : musicUrl
                        }
                  ...
                  >
                    {[
                      {
                        id: "vinyl" as MusicIconStyle,
                        emoji: "💿",
                        label: "Đĩa xoay",
                        desc: "Vinyl quay tròn",
                      },
                      {
                        id: "note" as MusicIconStyle,
                        emoji: "🎵",
                        label: "Nốt nhạc",
                        desc: "Nốt nhạc bay",
                      },
                      {
                        id: "miniplayer" as MusicIconStyle,
                        emoji: "🎧",
                        label: "Mini Player",
                        desc: "Thanh phát nhạc",
                      },
                      {
                        id: "wave" as MusicIconStyle,
                        emoji: "🌊",
                        label: "Sóng nhạc",
                        desc: "Waveform",
                      },
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background:
                            state.musicIconStyle === st.id ? "#fdf2f8" : "#fff",
                          cursor: "pointer",
                          textAlign: "center",
                          display: "flex",
                          flexDirection: "column",
              ...
              {activeTab === "templates" &&
                (() => {
                  ...
                  const filtered =
                    cat === "all"
                      ? TEMPLATE_PRESETS
                      : TEMPLATE_PRESETS.filter((t) => t.category === cat);
                  return (
                    <div
                      style={{
                        display: "flex",
                        ...
                        {filtered.map((t) => (
                          <button
                            ...
                            onClick={() => {
                              if (
                                confirm(
                                  `Áp dụng mẫu "${t.label}"? Nội dung hiện tại sẽ được thay thế.`,
                                )
                              ) {
                                dispatch({
                                  type: "LOAD",
                            ...
                            style={{
                              padding: 0,
                              border: `2px solid ${state.background === t.background ? t.accent : "#e5e7eb"}`,
                              borderRadius: 12,
                              overflow: "hidden",
                              background: "#fff",
                              cursor: "pointer",
                            ...
                            onMouseLeave={(e) => {
                              if (state.background !== t.background)
                                e.currentTarget.style.borderColor = "#e5e7eb";
                            }}
                          >
                            <div
        ...
        >
                  ...
                  {[
                    {
                      ...
                      label: "Nhân bản",
                      shortcut: "⌘D",
                      action: () => dispatch({ type: "DUPLICATE", id: sel.id }),
                    },
                    {
                      ...
                      shortcut: "",
                      action: () =>
                        dispatch({ type: "BRING_FORWARD", id: sel.id }),
                    },
                    {
                      ...
                      shortcut: "",
                      action: () =>
                        dispatch({ type: "SEND_BACKWARD", id: sel.id }),
                    },
                    // Sprint 29: Auto-align center buttons
                    {
                      ...
                      action: () =>
                        dispatch({
                          ...
                          id: sel.id,
                          changes: {
                            x: Math.round((state.width - sel.width) / 2),
                          },
                        }),
                    },
                    {
                      ...
                      action: () =>
                        dispatch({
                          ...
                          id: sel.id,
                          changes: {
                            y: Math.round((state.height - sel.height) / 2),
                          },
                        }),
                    },
                    {
                      ...
                      shortcut: "Del",
                      action: () =>
                        dispatch({ type: "DELETE_ELEMENT", id: sel.id }),
                      danger: true,
                    },
                  ].map((btn) => (
                    <button
                      ...
                      style={{
                        ...
                        background: "transparent",
                        cursor: "pointer",
                        color: (btn as { danger?: boolean }).danger
                          ? "#ef4444"
                          : "#4b5563",
                        display: "flex",
                        alignItems: "center",
          ...
          >
            <div
              style={{ position: "relative", paddingBottom: 68 }}
              data-canvas-export
            >
              {/* Sprint 31 → UX Polish: Thay ảnh nhanh — full-width bottom bar like Cinelove */}
              {(() => {
                const imageEls = state.elements.filter(
                  (e) => e.type === "image",
                );
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      style={{ width: 1, height: 44, background: "#e5e7eb" }}
                    />
                    <div
                      style={{
                        display: "flex",
              ...
              {state.elements.length === 0 && (
                ...
                >
                  ...
                  >
                    Chọn &quot;Văn bản&quot;, &quot;Hình ảnh&quot; hoặc
                    &quot;Tiện ích&quot; từ thanh bên trái để bắt đầu thiết kế
                    thiệp
                  </p>
                </div>
              )}
              ...
              >
                7app.online — Tạo thiệp miễn phí
              </div>
              <QuickImageBar
                elements={state.elements}
                selectedId={state.selectedId}
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                cursor: "pointer",
                color: snapToGrid ? "#f59e0b" : "#6b7280",
                padding: 0,
                display: "flex",
                transition: "color 0.2s",
                fontSize: 12,
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                cursor: "pointer",
                color: showMinimap ? "#8b5cf6" : "#6b7280",
                padding: 0,
                display: "flex",
                transition: "color 0.2s",
                fontSize: 11,
            ...
            {showGrid && (
              <select
                value={gridPattern}
                onChange={(e) =>
                  setGridPattern(e.target.value as "dots" | "lines" | "cross")
                }
                title="Kiểu lưới"
                style={{
                  padding: "2px 4px",
          ...
          {showMinimap && (
            <div
              style={{
                ...
                zIndex: 150,
                width: 160,
                height: Math.round(160 * (state.height / state.width)),
                background: "#fff",
                borderRadius: 10,
                boxShadow: "0 4px 20px rgba(0,0,0,0.15)",
                border: "1px solid #e5e7eb",
            ...
            >
              ...
              >
                {state.elements.map((el) => {
                  ...
                  const scaleY =
                    (160 * (state.height / state.width)) / state.height;
                  return (
                    <div
                      key={el.id}
                      style={{
          ...
          {showStats && (
            ...
            >
              ...
              <div>
                📐 Canvas: {state.width}×{state.height}px
              </div>
              <div>🔍 Zoom: {state.zoom}%</div>
              <div>📦 Elements: {state.elements.length}</div>
              <div>
                📝 Text:{" "}
                {state.elements.filter((e) => e.type === "text").length}
              </div>
              <div>
                🖼️ Image:{" "}
                {state.elements.filter((e) => e.type === "image").length}
              </div>
              <div>
                🧩 Widget:{" "}
                {state.elements.filter((e) => e.type === "widget").length}
              </div>
              <div>
                🔒 Locked: {state.elements.filter((e) => e.locked).length}
              </div>
              <div>
                💾 Status:{" "}
                {saveStatus === "saved"
          ...
          <button
            onClick={() => {
              ...
              (window as unknown as Record<string, unknown>).__musicPlaying =
                !cur;
            }}
            ...
            try {
              ...
              if (json.url) {
                dispatch({
                  type: "UPDATE_ELEMENT",
                  id: state.selectedId,
                  changes: { props: { ...selectedEl?.props, src: json.url } },
                });
              }
            } catch {
              alert("Upload thất bại, vui lòng thử lại.");
          ...
          onComplete={async (resultDataUrl: string) => {
            ...
            try {
              ...
              if (data.url && bgRemoveElementId) {
                dispatch({
                  type: "UPDATE_ELEMENT",
                  id: bgRemoveElementId,
                  changes: { props: { src: data.url } },
                });
              }
            } catch {
