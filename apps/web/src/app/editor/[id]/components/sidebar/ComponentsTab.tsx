...
interface ComponentsTabProps {
  ...
  /** Canvas-engine dispatch for inserting section presets */
  editorDispatch?: React.Dispatch<EditorAction>;
  /** Canvas-engine state for calculating insertion position */
...
export function ComponentsTab({
  ...
  const filteredLegacyPresets = SECTION_PRESETS.filter(
    (p) => sectionCat === "all" || p.category === sectionCat,
  );
  ...
  function insertLegacyPreset(presetId: string) {
    ...
    preset.elements.forEach((el) => {
      ...
      try {
        const tree = query.parseReactElement(reactEl).toNodeTree();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      >
        {/* Canvas-engine presets (shown when editorDispatch available) */}
        {useCanvasEngine &&
        ...
        {/* Legacy CraftJS presets (fallback when no editorDispatch) */}
        {!useCanvasEngine &&
          filteredLegacyPresets.map((preset) => (
            <button
              key={preset.id}
              onClick={() => insertLegacyPreset(preset.id)}
              style={{
                padding: 0,
                borderRadius: 10,
                border: "1px solid #e5e7eb",
            ...
            >
              ...
              >
                ...
                {preset.category === "invitation" && (
                  ...
                  >
                    <span style={{ fontSize: 16 }}>{"\u{1F48C}"}</span>
                    <div
                      style={{
                        width: "60%",
                        height: 4,
                ...
                {!["photo", "info", "timeline", "invitation"].includes(
                  preset.category,
                ) && (
                  <div
                    style={{
