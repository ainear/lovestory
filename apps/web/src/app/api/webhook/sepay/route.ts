...
export async function POST(req: NextRequest) {
  try {
    ...
    // Idempotency: reject duplicate webhook for same transaction
    if (transactionId) {
      const { data: existing } = await supabase
        .from("orders")
        .select("id")
        .eq("sepay_transaction_id", String(transactionId))
        .maybeSingle();
      if (existing) {
    ...
    supabase.auth.admin
      ...
      .then(({ data }) => {
        ...
        const name =
          data?.user?.user_metadata?.full_name || email?.split("@")[0] || "bạn";
        if (email) {
          sendPaymentConfirmedEmail(email, name, order.plan).catch(
            (e: unknown) =>
