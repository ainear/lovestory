...
export async function POST(req: NextRequest) {
  ...
  if (!isInternal) {
    ...
    const { createClient: createServerClient2 } =
  ...
  try {
    ...
    const isAllowedPhotoUrl = (url: string): boolean => {
      try {
    ...
    for (let i = 0; i < body.photoUrls.length; i++) {
      if (!isAllowedPhotoUrl(body.photoUrls[i])) continue;
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      ffmpeg.stderr.on("data", (data) => {
        ...
        if (timeMatch) {
          ...
          updateStatus(supabase, videoId, "processing", pct).catch(() => {});
        }
      });
    ...
    try {
      ...
      const thumbBuffer = await readFile(thumbnailPath).catch(() => null);
      if (thumbBuffer) {
        const thumbPath = `videos/${videoId}/thumb.jpg`;
        await supabase.storage.from("media").upload(thumbPath, thumbBuffer, {
    ...
    const thumbUrl =
      supabase.storage.from("media").getPublicUrl(`videos/${videoId}/thumb.jpg`)
        .data?.publicUrl || "";
